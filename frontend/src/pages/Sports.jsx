...
const Sports = () => {
  ...
  const sortedSportsArticles = Array.isArray(data)
    ? [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    : [];

  // Filter for top stories
  const topStories = sortedSportsArticles.filter((article) => article.topStory);

  // Use first 4 top stories for MidCard
  const top4 =
    topStories.length >= 4
      ? topStories.slice(0, 4)
      : sortedSportsArticles.slice(0, 4);

  const topIds = top4.map((a) => a.id);

  const restArticles = sortedSportsArticles.filter(
