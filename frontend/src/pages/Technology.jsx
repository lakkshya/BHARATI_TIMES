...
const Technology = () => {
  ...
  const { loading, error, data } = useFetch(
    `http://localhost:1337/api/articles/category/Technology/${language}`
  );
  ...
  const sortedTechnologyArticles = Array.isArray(data)
    ? [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    : [];

  // Filter for top stories
  const topStories = sortedTechnologyArticles.filter(
  ...
  const top4 =
    topStories.length >= 4
      ...
      : sortedTechnologyArticles.slice(0, 4);

  const topIds = top4.map((a) => a.id);

  const restArticles = sortedTechnologyArticles.filter(
    ...
    <div className="flex gap-4 !px-3 lg:!px-6 !py-10 bg-white">
      <main className="flex flex-col w-full gap-8 md:gap-10">
        <div>
          ...
          {translations[language].technology} {translations[language].news}
          </h2>
        </div>
        ...
        <section className="flex flex-col md:flex-row gap-10">
          ...
          <div className="flex flex-col gap-5 md:w-1/3">
            <h2 className="text-xl font-medium">{translations[language].youMayAlsoLike}</h2>
