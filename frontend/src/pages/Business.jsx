...
const Business = () => {
  ...
  const { loading, error, data } = useFetch(
    `http://localhost:1337/api/articles/category/Business/${language}`
  );
  ...
  const sortedBusinessArticles = Array.isArray(data)
    ? [...data].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))
    : [];

  // Filter for top stories
  const topStories = sortedBusinessArticles.filter(
  ...
  const top4 =
    topStories.length >= 4
      ...
      : sortedBusinessArticles.slice(0, 4);

  const topIds = top4.map((a) => a.id);

  const restArticles = sortedBusinessArticles.filter(
      ...
      <main className="flex flex-col w-full gap-8 md:gap-10">
        <div>
          <h2 className="text-xl font-medium inline tracking-widest">
            {translations[language].business} {translations[language].news}
          </h2>
        </div>
